//! Stage 1: extractor.
//!
//! Spawns the configured backend against a session JSONL file and parses
//! the resulting JSON array of draft entries. The model handles
//! parsing, role disambiguation, window selection, and dedup implicitly
//! — there is no Rust framework cognition.
...
pub struct DraftEntry {
    ...
    /// `[THINKING]:` / `[TOOL_INPUT:<name>]:` blocks, plus optional
    /// brief `[TOOL_RESULT:<name>]:` support blocks. Tool results are
    /// allowed only as short context around a stronger surrounding
    /// beat; the tokeniser will scrub PII downstream.
    pub quote: String,
    pub why: String,
}
...
pub fn extract_session(session_path: &Path, stage: &StageConfig) -> Result<Vec<DraftEntry>> {
    ...
    let raw = stage.run_extractor(&prompt, &session_dir)?;
    match parse_draft_array(&raw) {
        ...
        Err(_first_err) => {
            ...
            let retry = stage.run_extractor(&retry_prompt, &session_dir)?;
...
pub(crate) fn find_matching_bracket(s: &str, start: usize, open: u8, close: u8) -> Option<usize> {
...
mod tests {
    ...
    fn parse_error_does_not_leak_raw_output() {
        ...
        let secret_bearing = r##"[ {"title":"user alice shipped dp.sa.SECRETTOKEN","invalid": ""##;
