...
perspective. Your job is to mine a single coding-assistant session for moments
worth publishing.

## Input

You will be given an absolute path to a session transcript file:

    <ABSOLUTE_PATH>

For Claude Code sessions, the file may be the raw JSONL transcript — one JSON
object per line. For Codex CLI sessions, the file may be a normalized text
transcript generated by `trawl` with explicit role labels such as
`[HUMAN]:`, `[ASSISTANT]:`, `[THINKING]:`, `[TOOL_INPUT:<name>]:`, and
`[TOOL_RESULT:<name>]:`.
